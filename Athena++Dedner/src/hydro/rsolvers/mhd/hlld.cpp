...
#pragma omp simd simdlen(SIMD_WIDTH) private(wli,wri,spd,flxi)
  for (int i=il; i<=iu; ++i) {
    ...
    // Load psi separately
    Real psi_l = wl(IPSIW,i);
    Real psi_r = wr(IPSIW,i);
    ...
    // Interface psi (upwind based on contact wave)
    psi_interface = (spd[2] >= 0.0) ? psi_l : psi_r;
    ...
    ey(k,j,i) = -flxi[IBY] - psi_interface; //MODIFY FLUX FOR GLM
    ez(k,j,i) =  flxi[IBZ] - psi_interface; //MODIFY FLUX FOR GLM
    Real ch = pmy_block->phydro->ch_glm;
    Real chSquare = ch * ch;
    flx(IPSIC,k,j,i) = chSquare * bxi; //Add psi flux
