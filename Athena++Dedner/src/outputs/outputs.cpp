...
void OutputType::LoadOutputData(MeshBlock *pmb) {
  ...
  // GLM psi (divergence cleaning scalar) - primitive
  if (MAGNETIC_FIELDS_ENABLED) {
    if (ContainVariable(output_params.variable, "psi") ||
        ContainVariable(output_params.variable, "prim")) {
      pod = new OutputData;
      pod->type = "SCALARS";
      pod->name = "psi";
      pod->data.InitWithShallowSlice(phyd->w, 4, IPSIW, 1);
      AppendOutputDataNode(pod);
      num_vars_++;
    }
  }
  // GLM psi (divergence cleaning scalar) - conserved
  if (MAGNETIC_FIELDS_ENABLED) {
    if (ContainVariable(output_params.variable, "psi") ||
        ContainVariable(output_params.variable, "cons")) {
      pod = new OutputData;
      pod->type = "SCALARS";
      pod->name = "psi_cons";
      pod->data.InitWithShallowSlice(phyd->u, 4, IPSIC, 1);
      AppendOutputDataNode(pod);
      num_vars_++;
    }
  }
